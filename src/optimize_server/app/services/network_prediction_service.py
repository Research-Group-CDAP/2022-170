...
async def return_pod_names_NETWORK():
    mypath= os.getcwd() + '/app/util/datasets/NETWORK' 
    pods = [f.replace(".csv","") for f in listdir(mypath) if isfile(join(mypath, f))]
    return {"pod_name":pods}


async def make_pod_predictions_NETWORK(pod_name:str):
...
async def data_load_cpu_by_podname(pod_name:str):
    df = import_dataframe_from_csv("NETWORK/"+pod_name+".csv")
...
async def generate_cpu_utilization_graphs(dataframe):
    ...
    for col in df.columns:
        if col == 'timestamp':
            continue
        timeseries(df.index, df[col], 'index',col,'network')
...
async def export_predictions_dependecy(np_array,pod_name):
    ...
    export_dataframe_to_csv(df,'network',pod_name)
