...
async def return_podname_MEMORY():
    mypath= os.getcwd() + '/app/util/datasets/MEMORY' 
...
async def data_load_cpu_by_podname(pod_name:str):
    df = import_dataframe_from_csv("MEMORY/"+pod_name+".csv")
...
async def generate_cpu_utilization_graphs(dataframe):
    ...
    for col in df.columns:
        if col == 'timestamp':
            continue
        timeseries(df.index, df[col], 'index',col,'memory')
