...
const ExperimentSlideDrawer = (props) => {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_MONITORING_SUB_API_ENDPOINT}/experiment/fetchAllExperimentResults/${authState.user._id}`
      )
      .then((res) => {
        console.log(res);
  ...
  const generateYaml = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_MONITORING_SUB_API_ENDPOINT}/experiment/generateYamlFile/${podName}/${experimentType}`
      )
      .then((res) => {
  ...
  const executeExperiment = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_MONITORING_SUB_API_ENDPOINT}/experiment/executeExperiment`
      )
      .then((res) => {
  ...
  const responseAsJson = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_MONITORING_MAIN_API_ENDPOINT}/restartmonitoringserver`
      )
      .then((res) => {
        if (res.data.restart) {
          axios
            .get(
              `${process.env.REACT_APP_MONITORING_SUB_API_ENDPOINT}/experiment/responseAsJson`
            )
            .then((res) => {
              setIsExecuted(true);
              setLoadingStatus(false);
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          ...
          >
            ...
            <option value={"Pod_Termination_Dosent_Terminate_Others"}>
              Experiment 2
            </option>
            <option value={"All_the_applications_inthat_namespace_are_healthy"}>
              Experiment 3
            </option>
          </Select>
        </Grid>
        ...
        <Grid item xs={12}>
          {loadingStatus ? (
            ...
            <Alert severity="success">Succesfully Executed</Alert>
          ) : (
          ...
          <Grid item xs={12}>
            <Card className={classes.Card}>
              <CardContent>
                ...
                <Divider className={classes.Divider} />
                <br />
                If an instance of the application is terminated, all the
                applications in the given namespace must be healthy
              </CardContent>
            </Card>
          </Grid>
        </Grid>
        <Grid item xs={12}>
          {report && <ExperimentReport report={report} />}
        </Grid>
        <Grid item xs={12}>
          {experimentList.length && (
            <ExperimentsAccordion experimentList={experimentList} />
          )}
        </Grid>
      </Grid>
    </div>
